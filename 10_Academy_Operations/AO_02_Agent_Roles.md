...
firewall:
  modifies_physics_spine: false
  modifies_atlas: false
  modifies_nomos_theorem: false
---
...
- PROMOTE: adjudicate and approve artifact classification
...
### 4.3 Separation of Authority (Recusal + Two-Key)

**Recusal Rule (hard):**
- If any human (including Council/Founder) materially authored an artifact (text, structure, equations, or critical edits), they are **recused** from PROMOTE/ONTOLOGY_COMMIT for that artifact.

**Two-Key Rule (Founding Epoch waiver):**
Because we may be a single active operator (“1 squish”), we permit a minimum viable waiver that still blocks single-actor truth override:

For any promotion where the only available adjudicator is also the author, the artifact may be marked:

- `epistemic_status: PROVISIONAL_CANONICAL` (time-limited)
- AND must include a required signoff bundle in the log:

1) **Evaluation Engine report** (constraint + scoring output)  
2) **Red-Team Agent attempt log** (attempted falsification / exploit search)  
3) **Archivist checksum** (hash + immutable log entry)

**Deferred Adjudication (mandatory):**
- PROVISIONAL_CANONICAL items must be re-adjudicated by a non-recused Council/Adjudicator as soon as one exists.
- If re-adjudication fails, status is downgraded (or FROZEN) and downstream contagion rules apply.

This keeps velocity while preserving the firewall: *no unreviewed self-promotion becomes permanent truth*.
