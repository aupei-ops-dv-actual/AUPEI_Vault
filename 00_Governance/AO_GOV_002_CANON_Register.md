...
| `CANON-2026-016` | `TRIAGED` | `QUESTION` | Nested skill invocation in plugin runtime | Ministry of Doctrine / ζ + ω | ζ at plugin-install test | `/aupei:onboard` SKILL.md specifies: "After the verdict, run `/aupei:radio-check` inline as the final state confirmation. The radio check is the success signal that onboarding completed." Surfaced during test-(b) 2026-05-16: subagent correctly stub'd "Will follow with `/aupei:radio-check`" — subagent context doesn't have slash-command access. In real plugin runtime, the skill must ACTUALLY trigger the nested skill invocation. Plugin runtime support for nested-skill-call is UNVERIFIED — Cowork/Claude plugin docs not yet checked for whether one skill can invoke another mid-execution. Must validate at plugin install test before declaring pipeline production-ready. Three possible outcomes: (a) plugin runtime supports nested invocation cleanly — pipeline works as designed; (b) plugin runtime supports it with caveats (e.g., user-approval-per-call) — pipeline works with one extra click; (c) plugin runtime does NOT support nested invocation — pipeline needs design change: either merge onboard + radio-check into one skill, OR have onboard produce the radio-check output inline as part of its own report rather than as a separate skill call. | Resolved at first /aupei:onboard run in live plugin context — observed behavior decides which of (a)/(b)/(c) and feeds skill v2 |
| `CANON-2026-017` | `RESOLVED` | `TIGHTENING` | START_HERE.md vs /aupei:radio-check format drift | Ministry of Doctrine / ζ seat | ζ (C@ the Red) | Surfaced during adversarial discoverability test (test-a) 2026-05-16. ζ START_HERE.md Phase 4 carried legacy 7-field 10-2 format (Identity / Context Hold / Last Task / Current Task / Next Queued / State / Observation). The `/aupei:radio-check` SKILL.md carries the canonical 5-slot format (Signal / Loaded / Unknown / Flags / Verdict). Subagent's discovery trace: *"A cold instance could plausibly stop at START_HERE and miss the SKILL.md update. A pointer from START_HERE → SKILL.md would close the gap."* Caught only because the agent went deep enough to read both. **Repair 2026-05-16:** START_HERE.md Phase 4 rewritten to defer to `/aupei:radio-check` skill (5-slot format authoritative); legacy 7-field format explicitly marked deprecated with field-by-field mapping to new shape for legacy-doc compatibility; manual-walk exception clause added; logbook path corrected (`AUPEI/_live/10-2_LOGBOOK.md`, was missing `_live/`). | Resolved in this session post-test-(a) |
